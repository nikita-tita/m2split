...
- **М2-Оператор**: создание сделок, формирование реестров, отправка в банк, эмуляция ролей (QA)
- **АН-Админ**: управление своими сделками и агентами
- **Исполнитель (Агент/ИП/НПД)**: просмотр начислений, загрузка первички
- **Бухгалтер/Юрист**: просмотр документов и выплат (read-only+)
- **Банк**: системная интеграция (read-only)
...
### Эмуляция ролей (QA/тестирование)
- Переключение между ролями без re-login
- Баннер предупреждения при эмуляции
- Полное логирование всех действий
- Ограничение scope доступа (не расширяется)

### Инициатор сделки
- Фиксация создателя при создании сделки
- Неизменяемое поле (read-only для всех)
- Отображение в шапке карточки сделки
- Аудит chain: кто создал → кто изменил

### Stage-based Access Control
- Поля автоматически становятся read-only после утверждения
- Валидация переходов между статусами
- Подсказки о причинах недоступности полей
- Matrix-based права доступа

### Полная видимость полей БД
- Все поля модели данных доступны в UI
- Tooltips с информацией об источнике и доступе
- Нет "скрытых" полей
- Документированный маппинг UI ↔ БД

**Документация:**
- [EXTENDED_REQUIREMENTS.md](./EXTENDED_REQUIREMENTS.md) - полное ТЗ (8K+ слов)
- [IMPLEMENTATION_NOTES.md](./IMPLEMENTATION_NOTES.md) - план разработки
- [DATA_MODEL_TABLES.md](./DATA_MODEL_TABLES.md) - таблицы для Excel

## Безопасность

- Валидация всех входных данных
- Контроль прав доступа по ролям (RBAC + Stage ACL)
- Расширенный аудит с tracking эмуляции
- Idempotency для операций с банком (REGISTRY.id)
- API валидация заголовков (X-Act-As-Role, X-Act-As-Party)
