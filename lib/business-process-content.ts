...
export const businessProcessContent: Record<string, ProcessContent> = {
  ...
  overview: {
    ...
    description: 'М2 Split - это платформа для поиска и координации исполнителей заявок застройщика. Застройщик размещает базу контактов потенциальных клиентов, М2 находит агентов и АН для выполнения заявки. Когда исполнителей несколько, М2 автоматически распределяет комиссию КВН между ними в зависимости от их вклада в сделку. М2 получает единую комиссию от застройщика и выплачивает доли исполнителям.',
    steps: [
      'Застройщик размещает базу контактов (потенциальных клиентов) - это ЗАЯВКА на поиск исполнителей',
      'М2 предоставляет базу агентам и АН для работы с клиентами застройщика',
      'Агент/АН находит клиента из базы и создает сделку на бронирование объекта',
      'Если исполнителей несколько → они договариваются о распределении комиссии (сплит)',
      'Система рассчитывает комиссию КВН на основе договора М2 с застройщиком',
      'Застройщик переводит единую комиссию КВН на спецсчёт М2 за выполненную заявку',
      'М2 удерживает сервисный сбор за поиск исполнителей и координацию',
      'М2 распределяет остаток комиссии между исполнителями согласно их долям',
      'Исполнители предоставляют М2 первичные документы, М2 формирует пакет для застройщика',
    ],
    operations: {
      title: 'Финансовые потоки (модель маркетплейса исполнителей):',
      items: [
        'Застройщик → М2: единая комиссия КВН за выполнение заявки (поиск клиента и оформление сделки)',
        'М2 удерживает сервисный сбор за поиск исполнителей, предоставление базы контактов и координацию',
        'М2 → Исполнители: распределение комиссии по долям (сплит) в зависимости от вклада каждого',
        'Все операции проходят через спецсчёт М2 - застройщик не платит напрямую исполнителям',
        'Прямых финансовых отношений между застройщиком и исполнителями нет',
        'ЭКОНОМИЯ НДС: М2 распределяет через УСН/НПД исполнителей без НДС 20%, снижая налоговую нагрузку для АН',
      ],
    },
    accounts: {
