-- M2 Split - Tariff Card Import (FIXED for UUID schema)
...
JOIN projects p ON p.developer_id = c.id
WHERE c.name = 'Level Group' AND c.type = 'DEVELOPER'
  AND p.project_name LIKE '%Кавказский%'
LIMIT 1;
...
JOIN projects p ON p.developer_id = c.id
WHERE c.name = 'Level Group' AND c.type = 'DEVELOPER'
  AND p.project_name LIKE '%Павелецкая%'
LIMIT 1;
...
JOIN projects p ON p.developer_id = c.id
WHERE c.name = 'Level Group' AND c.type = 'DEVELOPER'
  AND p.project_name LIKE '%Римская%'
LIMIT 1;
...
SELECT
  ...
  ROUND(AVG(t.commission_total_percent), 2) as avg_commission,
